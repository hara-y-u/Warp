...
module.exports = Warp = (function() {
  ...
  Warp.prototype.handleWebSocketMessage = function(msg, id) {
    return console.log("client_" + id + "_" + msg.type + ":" + msg.data);
  };
  ...
  Warp.prototype.startStdinListener = function() {
    ...
    return this.stdin.on('end', function() {
      return console.log('status:stdin end');
    });
  };
    ...
    switch (chunk) {
      ...
      case "__endhtml__\n":
        ...
        this.sendWebSocketMessage({
          type: 'html',
          data: this.buf.html.join('')
        });
        this.buf.html = [];
        return;
    }
    if (this.mode === 'html' && /\S+/.test(chunk)) {
      return this.buf.html.push(chunk);
    }
  };
